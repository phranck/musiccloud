...
- **Jamendo-Endpoints bestätigt (live, sparsam):** `/tracks?album_id=<id>` liefert die Album-Tracks; `/tracks?artist_id=<id>&order=popularity_total` die Artist-Top-Tracks. (Nicht erneut proben — Rate-Limit.) Noch offen: `/artists?id=<id1>+<id2>` (Artist-Bilder für die Spalte) — am Execute EIN Call mit Pause.
