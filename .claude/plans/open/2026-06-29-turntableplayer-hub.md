...
- [x] **Step 5.1 — Knob rastet beim Drag ein.** Der Indikator folgt nicht mehr frei dem Zeiger, sondern snappt live auf die nächste der drei Stufen (STANDBY/33/45) via `speedFromAngle`, sodass er immer auf einer Stufe ruht und nie zwischen den Captions steht; beim Loslassen wird diese Stufe per `setSpeed` angewandt. `KnobDragState.angleDeg` → `snappedSpeed`; `animateIndicator` immer an (sanftes Gleiten zwischen Detents). Ein Zeiger ausserhalb des Label-Bogens bleibt an der nächstgelegenen erlaubten Stufe geklemmt.
...
- [x] **Step 5.3 — Tests.** Label-Beleuchtung (33/45 weiss bei Auswahl, ON orange bei power on, Standby unlit) und Detent-Einrasten beim Drag (Zwischenwinkel snappt auf Stufe, captionsloser Winkel bleibt an nächster Stufe). `stubKnobRect`-Helfer aus `dragKnobToAngle` extrahiert (DRY).
- [x] **Step 5.4 — Gates.** Biome 0 Fixes; `tsc --noEmit` 0 Fehler; react-doctor Full-Scan 0 Issues (alle 4 Workspaces); Frontend-Suite 52 Files / 322 Tests grün.
- [x] **Step 5.5 — Commit.** `Feat: knob detents + lit speed captions on the turntable (MC-071)`
...
- **F — Knob-Detents + Label-Beleuchtung (User, 2026-06-29, nach erster Umsetzung).** Der Knob rastet beim Maus-Drag an genau den drei Stufen STANDBY/33/45 ein (folgt nicht mehr frei dem Zeiger) und bleibt immer auf einer Stufe. **ON ist keine vierte Rast-Position, sondern reine Power-Anzeige:** das "ON"-Label leuchtet dezent orange, solange `power===On` (33 oder 45). Die Labels "33"/"45" leuchten weiss, wenn ihre Stufe gewählt ist; "STANDBY" hat keinen Leucht-Zustand. Umgesetzt in Einheit 5.
...
- [x] Einheit 5: Knob rastet beim Drag an STANDBY/33/45 ein (kein freies Folgen, ON = nur Power-Anzeige, Frage F); "33"/"45" leuchten weiss bei Auswahl, "ON" orange bei power on; `stubKnobRect`-Helfer (DRY); Tests + Gates grün (322 Tests). Commit.
