...
vi.mock("../services/cc/cc-share-response.js", () => ({
  buildCcAlbumPayload: vi.fn((album: Record<string, unknown>, tracks: unknown[]) =>
    Promise.resolve({ album: { ...album, tracks }, artistInfo: ARTIST_INFO }),
  ),
  buildCcArtistPayload: vi.fn((artist: Record<string, unknown>, topTracks: unknown[]) =>
    Promise.resolve({ artist: { ...artist, topTracks }, artistInfo: ARTIST_INFO }),
  ),
  ccTrackToPersistData: vi.fn((track: unknown) => track),
  toApiCcTrack: vi.fn((track: unknown) => track),
}));
...
describe("POST /api/v1/cc/resolve vinyl layout", () => {
  ...
  it("resolves and returns the containing album layout for a CC track", async () => {
    ...
    const response = await app.inject({
      ...
      payload: { selectedCandidate: "jamendo:track-1" },
    });
  ...
  it("resolves and returns the album layout for a CC album", async () => {
    ...
    const response = await app.inject({
      ...
      url: "/api/v1/cc/resolve",
      headers: { origin: "http://localhost:3000" },
      payload: { selectedCandidate: "jamendo-album:album-1" },
    });
  ...
  it("does not request a vinyl layout for a CC artist", async () => {
    resolveCcCandidate.mockResolvedValue({
      kind: "artist",
      artist: { jamendoId: "artist-1", name: "Jimmy Smith" },
      topTracks: [TRACK],
    });
    ...
    const response = await app.inject({
      ...
      url: "/api/v1/cc/resolve",
      headers: { origin: "http://localhost:3000" },
      payload: { selectedCandidate: "jamendo-artist:artist-1" },
    });
