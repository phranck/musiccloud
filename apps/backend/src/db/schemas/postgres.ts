...
export const analyticsEvents = pgTable(
  "analytics_events",
  {
    ...
    deviceModelCode: text("device_model_code"),
    isBot: boolean("is_bot").notNull().default(false),
    botName: text("bot_name"),
    botCategory: text("bot_category"),
    geoCountryCode: text("geo_country_code"),
    geoRegionCode: text("geo_region_code"),
    geoRegionName: text("geo_region_name"),
    geoCity: text("geo_city"),
    geoLatitude: real("geo_latitude"),
    geoLongitude: real("geo_longitude"),
    geoAccuracyRadiusKm: integer("geo_accuracy_radius_km"),
    geoTimeZone: text("geo_time_zone"),
    geoProvider: text("geo_provider"),
    geoDatabaseBuildAt: timestamp("geo_database_build_at", { withTimezone: true }),
  ...
  (table) => [
    ...
    index("idx_analytics_events_bot_occurred").on(table.isBot, table.occurredAt.desc()),
    index("idx_analytics_events_geo_country_occurred").on(table.geoCountryCode, table.occurredAt.desc()),
    index("idx_analytics_events_geo_city_occurred").on(table.geoCity, table.occurredAt.desc()),
    index("idx_analytics_events_geo_recent").on(table.occurredAt.desc(), table.geoCountryCode, table.geoCity),
    check(
      "chk_analytics_events_event_type",
      sql`${table.eventType} IN ('page_view', 'search_submitted', 'resolve_started', 'resolve_succeeded', 'resolve_failed', 'listen_on_clicked', 'similar_artist_clicked', 'popular_track_clicked', 'upcoming_event_clicked', 'player_started', 'player_paused', 'player_resumed', 'player_completed', 'player_unavailable', 'info_page_clicked', 'help_page_clicked', 'live_example_clicked', 'layered_footer_clicked', 'ui_click')`,
    ),
