...
import type { PersistCcTrackData } from "../../repository.js";

/**
 * Hits a live Postgres pointed at by `DATABASE_URL`. Exercises the slim CC
 * repository (`persistCcTrack` / `findCcTrackByShortId`) against the `cc_*`
 * tables from migration 0043.
 *
 * The fixture uses a random `jamendoId` per run so it never collides with
 * seeded data or a previous run. afterAll deletes every row it created in
 * FK-dependency order (`cc_short_urls` → `cc_tracks` → `cc_albums` →
 * `cc_artists`), because the CC FKs are `ON DELETE no action`.
 *
...
describe.skipIf(!process.env.DATABASE_URL)("CC repository (integration)", () => {
  ...
  afterAll(async () => {
    ...
    await pool.query(`DELETE FROM cc_albums WHERE jamendo_id = $1`, [jamendoAlbumId]);
    await pool.query(`DELETE FROM cc_artists WHERE jamendo_id = $1`, [jamendoArtistId]);
    await pool.end();
  });
