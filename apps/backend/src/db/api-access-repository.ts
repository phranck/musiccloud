 ...
 * credentials, usage, and audit trail. Shared by two route surfaces: the
 * admin dashboard (owner/admin moderation) and the developer-portal
 * self-service API (manage own projects and credentials), since both act on
 ...
 * @property createdByAdminId - Admin who created the client directly, or `null` when the developer created it.
 */
 ...
 * @property eventType - e.g. `"registration_created"`, `"token_rotated"`.
...
export interface ApiAccessRepository {
  ...
  /**
   * Creates a new active token for a client. The id is generated by the
   * implementation. Only the hash and the display prefix are stored, so the
   * caller's own response is the only place the issued token ever exists.
   */
