...
/**
 * An approved API-client ("app") DTO. Every read resolves the owning
 * account's tier alongside the client so consumers always see both the raw
 * per-key overrides and the effective limits (MC-100).
 *
 ...
 * @property requestsPerMinute - Per-key override, or `null` to inherit the account tier's limit ("custom tier" when set).
 * @property requestsPerDay - Per-key override, or `null` to inherit the account tier's limit.
 * @property tierId - The owning account's assigned tier id, or `null` when unassigned.
 ...
 * @property effectiveRequestsPerMinute - Resolved limit (`override ?? tier ?? fallback`) enforced by `authenticatePublic`.
 * @property effectiveRequestsPerDay - Resolved daily limit (same precedence).
...
export interface ApiAccessRepository {
  ...
  /**
   * Creates a new client. The id is generated by the implementation;
   * `status` starts `"active"`. Omitted rate-limit fields are stored as
   * `NULL` — the client inherits the owning account's tier limits until an
   * admin sets an explicit override.
   */
   ...
   * client inherits the tier limit again.
   */
  updateApiClient(
    id: string,
   ...
   * The returned client carries the resolved
