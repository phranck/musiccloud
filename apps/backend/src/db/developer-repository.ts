...
export interface DeveloperRepository {
   ...
   * `lastLoginAt` start `null`; `status` defaults to `"active"`.
   *
   ...
   *   fields. `tierId` assigns a tier right at signup (the pricing page's
   *   Subscribe flow, MC-101) — the route layer validates that it references
   *   an enabled tier; omitted/null starts the account unassigned.
   ...
   * Includes the count of active API clients each account owns plus the
   * assigned tier's display fields (name + enabled flag) for the dashboard
   * overview — both `null` when no tier is assigned.
   *
   * @returns Array of account DTOs, each extended with `clientCount`,
   *   `appName`, `tierName` and `tierEnabled`.
   */
   ...
   *   `status` are all optional. Passing `tierId: null` explicitly removes
   *   the tier assignment (the route layer validates that non-null ids
   *   reference an enabled tier).
