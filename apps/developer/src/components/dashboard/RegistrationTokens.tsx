...
export function RegistrationTokens({
  ...
  return (
    <div className="flex flex-col gap-3 mt-3" data-registration-tokens>
      <p className="text-nav text-fg-muted">Keys</p>
      ...
      {current.length === 0 && <p className="text-nav text-fg-muted">No key yet.</p>}
            ...
            <li key={token.id} className="flex flex-col gap-1">
              ...
              {token.status === ApiTokenStatus.Active && (
                <>
                  ...
                  {pendingRevokeId !== token.id && <p className="text-nav text-fg-muted">{ROTATE_WARNING}</p>}
                </>
              )}
            </li>
          ))}
